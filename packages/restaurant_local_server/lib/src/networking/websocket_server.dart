...
class WebSocketServer {
  ...
  WebSocketServer({required this.config, this.eventHandlers});
  ...
  Future<void> _handleWebSocketConnection(HttpRequest request) async {
    try {
      ...
      if (config.maxClients > 0 && _clients.length >= config.maxClients) {
        _log(
          'Rejecting connection: Maximum clients (${config.maxClients}) reached',
        );
        request.response
  ...
  Future<void> disconnectAllClients([String? reason]) async {
    ...
    for (final clientId in clientIds) {
      await _closeClient(
        clientId,
        reason ?? 'Server disconnecting all clients',
      );
    }
  }
}
